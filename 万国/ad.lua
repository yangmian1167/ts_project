...
function _monster()
	...
	while d('采矿-放大镜') and 打野次数记数 < _UI.打野次数 and _UI.打野 do
		...
		local i = 1
		while i < 60 and ( d('打野-正在行军') or d('打野-正在战斗') )  and _UI.打野  and d('采集-已经有队伍') and d('采集-已经有队伍-正确') do
		    delay(1)
		    if d('打野-正在挖矿') then
		        _UI.打野 = false
		    end
...
function _加天赋()
    ...
    if d('联盟-展开联盟-天赋',true,1,5) then
        local 加天赋位置= {{480, 504, 0xfffffc},{470, 692, 0xfffff1},{353, 686, 0xccb167},{353, 686, 0xccb167},{353, 686, 0xccb167},{314, 415, 0xfffffb},{314, 415, 0xfffffb},{264, 517, 0xf0e1b7},{192, 344, 0xd8bd71}}
        local i = 0
...
pages=        
    {
        { 
            {["type"] = "Label",["color"] = "255,30,2",["size"] = "12",["align"] = "left",["text"] = "广告内容",},
            {["type"] = "Edit",["color"] = "38,38,38",["prompt"] = "广告内容",["size"] = "15",["id"] = "ad",["align"] = "left",["text"] = "这是一条广告",["kbtype"] = "default",},
        }
    },
} 
...
function main()
	...
	while true do
		...
		__game={}
        if false or AccountInfoBackad( getOnlineName() ) then
           	log(_UI);
    		game()
        end
        clearOneAccount();
