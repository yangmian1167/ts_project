...
t['弹窗-隐私保护-同意并继续']={0x0ea8fc, "0|0|0x0ea8fc,-136|-8|0xf7e1c6,-362|-406|0xd30000,-378|-402|0xf7e1c6",90,176,88,1130,662}

t['弹窗-其他设备登录-确定']={ 0x1274ba, "-103|-21|0x00d4fe,130|24|0x00b5f0,42|-10|0x1274ba,135|-18|0x00c1f3", 90, 540, 494, 806, 570 } --多点找色
t['弹窗-其他设备登录-白色字']={ 0x5293ae, "-3|19|0xaccbd9,279|1|0xe0ecf1,288|20|0x5e9cb6,290|18|0xdfebf0", 90, 501, 323, 816, 365 } --多点找色
function _Tips()
	log('tips弹窗处理↓')
	local tips = {
	...
	for k,v in pairs(tips)do
		...
		elseif v == '弹窗-其他设备登录-白色字' and v == '弹窗-其他设备登录-确定' then
			log('其他设备登录')
			return '顶号'
...
function _升级子程序()
    ...
    while (d("升级-升级按钮") or d("升级-前往")) and i < 20 do
		...
		elseif d("升级-前往",false,1,2)then
			x , y = findMultiColorInRegionFuzzy( 0x00cdff, "87|17|0x00b7f3", 90, 655, 541, 1137, 642, {orient = _orient} )---主城两个建筑条件都点
			click(x , y)
			if _orient >= 3 then
				_orient = 1
			end	
...
t['第二建筑-钎子iphone7']={ 0x0a83bc, "-14|-17|0x2da6dc,-27|22|0x006397,-20|19|0xaed5ed,23|3|0xd6ebec", 90, 792, 305, 942, 499 } --多点找色
t['第二建筑-建筑工人管理-界面']={0xd3d2cb, "0|0|0xd3d2cb,-488|-8|0x858278",90,546,51,1194,142}
    ...
    t['第二建筑-建筑工人管理-界面-直接雇佣']={ 0xffb700, "-146|-11|0xffbe00,5|27|0xffa600", 90, 926, 363, 1147, 463 } --多点找色
function _build_two()
	log("第二建筑")
	升级次数 = 升级次数 + 1
	if 升级次数 > 4 then
	...
	delay(1)
	if d('第二建筑-钎子',1,2) or d('第二建筑-钎子iphone7',1,2)then
	    if d('第二建筑-建筑工人管理-界面')then
	        d('第二建筑-建筑工人管理-界面-黄色雇佣按钮',true,1,2)
	        d('第二建筑-建筑工人管理-界面-直接雇佣',true,1,2)
...
t['造兵-准备训练2']={ 0x4dbdef, "5|-11|0xdfebee,-276|-13|0xe0ecee,-272|-1|0x4cc0ef,-137|1|0xc4e6bf", 90, 184, 623, 601, 664 } --多点找色
...
--	}	
	...
	for k,v in ipairs(兵种) do
		if _UI.造兵[v[1]] then
		    log('准备造'..v[1])
			    click(v[2], v[3],2)
			if not (  d('造兵-准备训练') ) and not (  d('造兵-准备训练') ) then
			    click(v[2], v[3],2)
			end
...
t['收集-金币']={ 0xdd7d00, "-6|3|0xfae900,-6|-7|0xffee00,18|-4|0xfef500,16|5|0x994a00", 90, 50,20,1250,736 } --多点找色
...
function _Collect()
	...
	d('收集-帮助',true,2)
	if 收集次数 < 2 then
    	local 收集 = {"收集-玉米","收集-木材","收集-石头","收集-握手","收集-木材黄","收集-金币"}
...
t['英雄-放大镜1'] = { 0x3cb3e8, "1|13|0xd7edf8,-7|41|0x00639c,-282|-3|0x41b7ec,-274|-1|0xcce7f4,-282|-24|0xcfe4ed,11|-16|0xdbe9f1", 90, 135, 240, 605, 570 } --多点找色
...
function _Hero()
	log("英雄")
	click(314,344,2)
	if d("英雄-放大镜",true,1,2)or d("英雄-放大镜1",true,1,2)then
		local _opentimes = 0
...
t['打野-创建部队']={ 0x1188c9, "108|27|0x59bae6,-34|-4|0x00d0ff,139|34|0x00b8f4,103|-1|0x0c92d1", 90, 948, 105, 1161, 197 } --多点找色
	...
	t['打野-创建部队-驻扎2']={ 0x008ec2, "2|5|0xffffff", 90, 1269, 62, 1312, 536 } --多点找色
--    t['打野-创建部队-行军']={0xb67016, "0|0|0xb67016,-90|-10|0xffba00,63|22|0xffa500",90,926,64,1319,394}
    t['打野-创建部队-行军']={ 0xb67016, "-91|-11|0xffbc00,69|25|0xffa700,77|-162|0xf9dbba,-36|-77|0xff9b1d", 90, 923, 13, 1168, 727 } --多点找色
...
t['打野-正在行军']={0xffffff, "0|0|0xffffff,2|5|0x4ef96a,2|-17|0x4ffd6c,7|-5|0x0c9900,6|-5|0xffffff",90,1276,165,1324,458}
t['打野-正在行军-返回']={0xdb8100, "0|0|0xdb8100,6|2|0xfefefd,7|14|0xb45d00",90,1289,235,1314,269}
t['打野-正在行军-返回-iphone6']={0xc97612, "0|0|0xc97612,5|5|0xfdfaf7,5|10|0xb25c01",90,1289,235,1314,269}
...
t['打野-体力不足2-iphone7']={ 0xfcca00, "0|12|0x987a00,-15|16|0x033a51,-39|13|0x4fe20b,-18|-6|0xff9b1d", 90, 396, 126, 531, 179 } --多点找色
t['打野-体力不足-每日领取']={ 0x208600, "-33|-7|0x268c08,-98|-20|0x00d10f,71|21|0x00b70e,77|-18|0x00d00f", 90, 911, 213, 1135, 321 } --多点找色
---------------------
--t['打野-野外-行军']={ 0xb67016, "-23|0|0xb67016,-95|-7|0xffbc00,58|29|0xffa500,61|-6|0xffbb00", 90, 958, 146, 1157, 745 } --多点找色
--t['打野-野外-行军-优势明显']={ 0x20ab33, "42|0|0x50bb5c,127|4|0x23ab35,13|33|0xffbc00,145|58|0xfeab00", 90, 958, 146, 1157, 745 } --多点找色
--t['打野-野外-行军-敌弱我强']={ 0x31b041, "-144|1|0x1daa30,-136|31|0xffbc00,20|55|0xfead00,-51|40|0xb67016", 90, 965, 145, 1152, 746 } --多点找色


t['打野-驻扎部队']={ 0x008ec2, "0|8|0xffffff,7|6|0xffffff,3|13|0x008ec2", 90, 1289, 172, 1309, 446 } --多点找色
t['打野-驻扎部队-返回']={ 0xe7b226, "34|14|0xd89c10,8|31|0xf5f1e5,27|-8|0xf7f4e9", 90, 550, 342, 840, 550 } --多点找色
function 驻扎部队回城补给()
	log("回城补给")
	if d('打野-驻扎部队',true) then
		delay(2)
		if d('打野-驻扎部队-返回',true) then
			return
		end	
	end
end
...
-- 		while i < 300 and ( d('打野-正在行军')  or d('打野-正在战斗') )   and d('采集-已经有队伍') and d('采集-已经有队伍-正确') and _UI.打野 do
		...
		if d("采矿-搜索",true,1,3)then
			...
			if d('打野-搜索',true)then
    			...
    			d("采矿-搜索-攻击",true,1,3)
    			if d("打野-创建部队",false,1,2)  then
    			    d('打野-创建部队-驻扎',true,1,2)	
    			    d('打野-创建部队-行军',true,1,3)
    			end
    			d("打野-创建部队",true,1,3)  
    			d('打野-创建部队-正在回来敌弱我强',true,1,2)
				d('打野-创建部队-驻扎2',true,1,2)
    			d('打野-创建部队-行军',true,1,2)
				if d('打野-体力不足2-iphone7') then
					if d('打野-体力不足-每日领取',true) then
						click(1136,89)
					else
						log('体力不足,结束打野','all')
						click(1134,85)--关闭体力弹窗
						click(123,439)--不点行军
						驻扎部队回城补给()
						_UI.打野 = false
						return
...
t['采集-采集前调低']={0x0eab00, "0|0|0x0eab00,-42|-1|0x0f74be,63|89|0x00c3ff,244|-1|0x1073bd",90,20,363,1335,606}
    ...
    t['采集-已经有队伍-4']={0xfafafa, "0|0|0xfafafa,1|-9|0xffffff,-5|-3|0xf9faf9",90,1261,167,1276,186}
     t['采集-已经有队伍-5']={ 0xf6f6f3, "5|0|0xfbfcfa,0|4|0xf9f9f9,5|8|0xffffff", 90, 1263, 167, 1275, 186 } --多点找色
    --最大值
    ...
    t['采集-已经有队伍-5-max']={ 0xf6f6f3, "5|6|0xffffff,1|10|0xfefefe,5|0|0xfbfcfa", 90, 1279, 166, 1293, 186 } --多点找色
    
t['采集-行军-有兵']={0x0aba00, "0|0|0x0aba00,0|-1|0x0bc200",90,574,134,1107,584}
t['采集-行军-有兵2']={ 0x0e9400, "", 90, 663, 192, 678, 523 } --多点找色
t['采集-行军-有兵3']={ 0xb67016, "-63|-15|0xffbe00,-46|18|0x73563c,-50|37|0x7f573e,-48|25|0x4990bb", 90, 841, 610, 1001, 692 } --多点找色
...
function _Collection()
	...
	if _UI.采集.key then
--		local i = 1
--	    while i < 300 and ( d('打野-正在战斗') or d('打野-正在行军-返回',true,1,5) or d('打野-正在行军-返回-iphone6',true,1,5) )  
--	    and d('采集-已经有队伍') and d('采集-已经有队伍-正确') do
--			delay(1)
--		    i=i+1
--		end	
		驻扎部队回城补给()
	    ...
	    if d('采集-已经有队伍') and d('采集-已经有队伍-正确') then
            local dwshu_min = { "采集-已经有队伍-1", "采集-已经有队伍-2", "采集-已经有队伍-3", "采集-已经有队伍-4", "采集-已经有队伍-5" }
            local dwshu_max = { "采集-已经有队伍-1-max", "采集-已经有队伍-2-max", "采集-已经有队伍-3-max", "采集-已经有队伍-4-max" , "采集-已经有队伍-5-max" }
			if  DWs(dwshu_min) == (DWs(dwshu_max)-1) then
				_UI.采集.key = false
	            return true
	        elseif DWs(dwshu_min) == DWs(dwshu_max) then
	            _UI.采集.key = false
	            return true
	        end
	    end   
		if d('采矿-放大镜',true,1,2)then
			if tonumber(_UI.采集.种类) >= 5 then
				 times_random = getNetTime()
				 times_random =  tonumber(string.sub(times_random,-1))
				if times_random == 1 or times_random == 4 or times_random == 7 then 
					click( 采集位置[1][1],采集位置[1][2],2 )   --粮
				elseif times_random == 2 or times_random == 5 or times_random == 8 then 
					click( 采集位置[2][1],采集位置[2][2],2 )   --木
				elseif times_random == 3 or times_random == 6 or times_random == 9 then 
					click( 采集位置[3][1],采集位置[3][2],2 )   --石
				elseif times_random == (0) then 
--					click( 采集位置[4][1],采集位置[4][2],2 )   --金
					click(1231,301) --点空白取消
				end	
			else
			...
			if _UI.矿等级 == '默认' then
				...
				采集等级选择 = tonumber(string.match(_UI.矿等级,'%d+')) - 1
				if d("采矿-搜索",false,1,3)then
					d('采集-采集前调低',true,1,1)
					if 采集等级选择 == 0 then
					else	
...
--    			    end
				...
				if d("采矿-行军")  then
    			    ...
    			    if d("采矿-行军") or d('采矿-行军2') then
    			        if d('采集-行军-有兵') or d('采集-行军-有兵2')or d('采集-行军-有兵3') then
    			            d("采矿-行军",true)
    			            d('采矿-行军2',true)
                        else
                            _UI.采集.key = false
...
function read_mail()
    ...
    if d('邮件-邮件',true,1,4)then
        ...
        local 派遣次数 = 0
        while d('邮件-邮箱界面')  and _UI.搜索村庄 and os.time() - mailtime < 50 do
...
t['日历-有奖励-2']={ 0xe30000, "-21|0|0xe30000", 90, 1296, 86, 1330, 116 } --多点找色
...
t['日历-奖励界面-左侧面板']={0x533f21, "0|0|0x533f21,119|-29|0xde0000",90,163,45,440,690}
	...
	t['日历-奖励界面-红蓝2']={ 0xe20000, "-8|2|0x265fa4", 90, 462, 159, 1174, 184 } --多点找色
	t['日历-奖励界面-红黄']={ 0xe20000, "-8|5|0xdfcba4,5|-3|0xde0000", 90, 461, 208, 1171, 246 } --多点找色
...
t['任务-联盟互动']={0x00bcfc, "0|0|0x00bcfc,-816|-26|0xeeb5a7,-823|-36|0xa65c4a,-780|-62|0xe3b300,-852|-63|0xd9af01",90,177,246,1147,674}
    ...
    t['任务-知识之力']={0x1274ba, "0|0|0x1274ba,-750|-58|0xfdee1c,-756|-57|0x834910,-774|2|0x1ab2ff,-780|-10|0xfeffff",90,178,267,1162,678}
        ...
        t['任务-英雄之力-学习-使用']={0x1274ba, "0|0|0x1274ba,-85|-18|0x00d0fd,53|23|0x01aeeb",90,903,210,1137,305}
...
t['任务_科技时代']={0x00baf9, "0|0|0x00baf9,-811|-26|0x37def8,-815|9|0x03693d,-780|-51|0xffde90,-810|-20|0x0f3073",90,2,2,2,2}

function _神秘商店()
    delay(1)
    local i = 0
    while i < 2 and d('任务-神秘商人界面') do
        local redx = 385
        for ii = 1,3 do
            for i=0,3 do
                t['商店购买-玉米1']={ 0xfac42b, "-3|0|0xff9e16,-10|6|0x56b70c,-4|14|0x225d02,-23|-6|0xffbe00,77|21|0xfda200", 90, 392, 204, 1138, 675 } --多点找色
				t['商店购买-玉米2']={ 0xffe72c, "1|9|0x3d9c09,-5|14|0x235a03,-19|-6|0xffbe00,80|22|0xf79a00,5|-2|0xfffd60", 90, 392, 204, 1138, 675 } --多点找色
				t['商店购买-玉米3']={ 0xfffb69, "4|-3|0xfdf456,-7|9|0x338703,2|9|0x296700,-15|-9|0xffbe00,84|16|0xffa500", 90, 185, 264, 1159, 673 } --多点找色
                t['商店购买-木材1']={ 0x9a4625, "5|-6|0xfbe1bb,10|-5|0x752d09,8|-10|0xbb521b,-8|-19|0xfebf00,98|14|0xea8b00", 90, 392, 204, 1138, 675 } --多点找色
				t['商店购买-木材2']={ 0xce7f4d, "2|-2|0x9c502c,4|-7|0xfac791,9|-7|0x81320a,-11|-16|0xffbe00,88|9|0xfea500", 90, 392, 204, 1138, 675 } --多点找色
				t['商店购买-木材3']={ 0x792f0a, "-5|4|0x92370b,-8|2|0xfce7c4,-14|8|0x954020,-22|-9|0xffbe00,82|18|0xfea500", 90, 392, 204, 1138, 675 } --多点找色
                local i = 0
                while (  d('商店购买-木材1',true,1,2) or 
						d('商店购买-木材2',true,1,2) or 
						d('商店购买-木材3',true,1,2) or 
						d('商店购买-玉米1',true,1,2) or 
						d('商店购买-玉米2',true,1,2) or 
						d('商店购买-玉米3',true,1,2) ) and i < 4 do
                    i=i+1
                end
            end
            moveTo(757, 581,759, 376,2,500)
            delay(2)
        end
		log('执行一次')
        if d('任务-神秘商人-免费刷新',true)then
--        elseif d('任务-神秘商人-收费刷新') then
--            return
        end
        i=i+1
    end
end
t['任务-太阳勇士-日落峡谷']={ 0xffffff, "-1|-149|0xffffd9,-16|-173|0xff321a,-103|-21|0x066a94,-50|-2|0xffffff", 90, 696, 190, 942, 519 } --多点找色
t['任务-太阳勇士-日落峡谷-挑战']={ 0x1274ba, "-88|-20|0x00d1ff,65|20|0x00b8f4", 90, 538, 179, 1171, 745 } --多点找色
t['任务-太阳勇士-日落峡谷-挑战-确定']={ 0x1274ba, "-97|-25|0x00d0ff,64|11|0x00b9f5,-72|-74|0x6bfa38,-59|-55|0x2fee22", 90, 532, 593, 790, 721 } --多点找色
function _task()
...
--		t['任务_点击任务_王者任务']={0x1274ba, "0|0|0x1274ba,-794|-11|0xf6d58e,-814|-13|0xfa2a39,-798|-52|0xd6dff5,-815|13|0xd32e36",90,165,250,1162,681}
		t['任务_点击任务_王者任务']={0x1274ba, "0|0|0x1274ba,-794|-11|0xf6d58e,-814|-13|0xfa2a39,-798|-52|0xd6dff5,-815|13|0xd32e36",90,2,2,2,2}
		t['任务-知识之力']={ 0x1375ba, "31|8|0x00b8f4,-821|-48|0xffff4c,-826|-10|0x0858b7,-798|-15|0xadb9ca,-774|4|0x4e1c55,-786|-54|0x0397ff", 90, 210, 266, 1131, 677 } --多点找色
--		t['任务-联盟互动']={0x00bcfc, "0|0|0x00bcfc,-816|-26|0xeeb5a7,-823|-36|0xa65c4a,-780|-62|0xe3b300,-852|-63|0xd9af01",90,177,246,1147,674}
		t['任务-联盟互动']={0x00bcfc, "0|0|0x00bcfc,-816|-26|0xeeb5a7,-823|-36|0xa65c4a,-780|-62|0xe3b300,-852|-63|0xd9af01",90,2,2,2,2}
--		t['任务-王国贸易']={ 0x1375bb, "-794|-18|0xe49a88,-810|1|0x044b99,-767|4|0x481d51,-794|-62|0x0155a6,-770|-40|0xf5f4f6", 90, 203, 268, 1141, 671 } --多点找色
		t['任务-王国贸易']={ 0x1274ba, "-65|-8|0x00ccfe,40|11|0x00b6f1,-794|-16|0xe8a391,-780|-2|0x28233b,-766|12|0x3e1746,-793|-57|0x004489", 90, 185, 264, 1159, 673 } --多点找色
		t['任务-太阳勇士']={ 0x1274ba, "-68|-12|0x00d2ff,41|10|0x00b7f3,-792|-28|0xd7cdbd,-796|-45|0xba0303,-828|-44|0xf0a925,-775|-6|0x966743", 90, 178, 273, 1154, 678 } --多点找色
	    local taskArrs = {'任务_点击任务_王者任务','任务-知识之力','任务_科技时代','任务-王国贸易' ,'任务_采集类','任务-太阳勇士'}
	    ...
	    while i < 6 do
	        ...
	        for k,v in ipairs( taskArrs )do
	            if d(v,true,2) then
					...
					elseif k == 6 then
						if d('任务-太阳勇士-日落峡谷',true) then
							if d('任务-太阳勇士-日落峡谷-挑战',true) then
								d('任务-太阳勇士-日落峡谷-挑战',true) 
								delay(3)
								if d('任务-太阳勇士-日落峡谷-挑战-确定',true) then
								else
									click(613,615)
									d('任务-太阳勇士-日落峡谷-挑战-确定',true)
								end
							end
						end	
					end
...
t['设置-执正官资料-设置']={0x10363f, "0|0|0x10363f,-145|8|0xdf0e29,-283|-3|0xffce35",90,230,482,1108,657}
    t['设置-执正官资料-通用设置']={0xf1e5cf, "0|0|0xf1e5cf,-244|-7|0xfef7a9,7|394|0xff3842",90,207,91,667,686}
        t['设置-执正官资料-通用设置-设置界面']={0xd2d1ca, "0|0|0xd2d1ca,-1016|-4|0xb5b2a6,-1023|88|0x05445e,28|86|0x044661",90,116,6,1232,231}
            t['设置-执正官资料-通用设置-设置界面-帮助']={0xa6f0ff, "0|0|0xa6f0ff,-22|-107|0xfafcfd,-216|-106|0x81e430,-374|-104|0x77d1ff,-570|-103|0xc05cff,-728|-102|0xffa92e",
                90,187,90,1171,646}
...
-- 563,78    研究院的位置
t['研究院-正在升级']={0xe0a40f, "0|0|0xe0a40f,-15|-28|0xf6f2ed,-157|-1|0x097fb6",90,354, 275, 758, 502}
t['研究院-正在研究']={0xe6aa14, "0|0|0xe6aa14,-21|-25|0xf0f0ef,-166|-21|0x097cb3,116|-52|0x59c1f0,92|-23|0xcfe3eb",90,354, 275, 758, 502}
t['研究院-药瓶']={ 0x006598, "0|-24|0xe7f3f8,-44|-9|0xcee5e8,28|-38|0xd1e3ec,19|-24|0x0c8ac3", 90, 575, 279, 803, 515 } --多点找色
...
t['远征-创建部队-行军']={ 0xb67016, "-93|-11|0xffbe00,62|31|0xffa500,69|-9|0xffbe00", 90, 949, 23, 1155, 746 } --多点找色
function _远征()
    远征次数 =  远征次数 + 1
    if 远征次数 > 1 then
        _UI.远征 = false
...
t['有可造建筑']={ 0x6e7992, "1|9|0x1cf300,5|10|0x18ee00,5|14|0x74819b,2|12|0x1adb01", 90, 4, 3, 1329, 743 } --多点找色
function 全面升级()
	if d('有可造建筑',true) then
		if d("升级-升级箭头",true,1,2) or d('升级-升级箭头时代',true,1,2) or d('升级-升级箭头时代-iphone7',true,1,2) or d('升级-升级箭头-车厂',true,1,2)
		or d('升级-升级箭头时代-iphone7-9',true,1,2)
		then
		end
